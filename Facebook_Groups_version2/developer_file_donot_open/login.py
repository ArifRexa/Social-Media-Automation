...
def login1(iid, pas):
    ...
    data = "adid=" + str(uuid.uuid4()) + "&format=json&device_id=" + str(
        uuid.uuid4()) + "&email=" + iid + "&password=" + pas + "&generate_analytics_claims=1&community_id=&cpl=true&try_num=1&family_device_id=" + str(
    ...
    res = checker(requests.post("https://b-graph.facebook.com/auth/login", headers=head, data=data).json())
    return res
...
def checker(po):
    if 'session_key' in po:
        open('Facebook_Groups_version2/developer_file_donot_open/token.txt', 'a').write(po["access_token"])
        ...
        checker_return = True
        if checker_return:
            return "Facebook Account Is In Checkpoint"
        exit()
    else:
        print(' \033[1;35m SomeThing Went Wrong Maybe Email Or Password Is Wrong\033[0m')
        checker_return = True
        if checker_return:
            return "SomeThing Went Wrong Maybe Email Or Password Is Wrong"
        exit()
